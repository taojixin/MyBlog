...
<h2>模板语法</h2>
<h3>1.插值语法</h3>
<p><strong>功能</strong>：用于解析标签体内容。</p>
<p><strong>写法</strong>：{{xxx}}，xxx是js<strong>表达式</strong>，且可以直接读取到data中的所有属性。</p>
<h3>2.指令语法</h3>
<blockquote>
<p>指令有很多，v-xxx，如v-bind。</p>
</blockquote>
<p><strong>功能</strong>：用于解析<strong>标签</strong>（包括：标签属性、标签体内容、绑定事件.....）。</p>
<p><strong>举例</strong>：v-bind:href=&quot;xxx&quot; 或  简写为 :href=&quot;xxx&quot;，xxx同样要写js表达式，且可以直接读取到data中的所有属性。</p>
<pre><code class="language-html">&lt;!DOCTYPE html&gt;
&lt;html&gt;
	&lt;head&gt;
		&lt;meta charset=&quot;UTF-8&quot; /&gt;
...
<h2>数据绑定</h2>
<blockquote>
<p>Vue中有2种数据绑定的方式：<strong>单向绑定</strong>(v-bind)和<strong>双向绑定</strong>(v-model)。</p>
</blockquote>
<ul>
...
<p><strong>Vue中的数据代理</strong>：通过vm对象来代理data对象中属性的操作（读/写）。</p>
<p><strong>Vue中数据代理的好处</strong>：更加方便的操作data中的数据</p>
<p><strong>基本原理</strong>：通过Object.defineProperty()把data对象中所有属性添加到vm上。为每一个添加到vm上的属性，都指定一个getter/setter。在getter/setter内部去操作（读/写）data中对应的属性。</p>
<pre><code class="language-html">&lt;!DOCTYPE html&gt;
&lt;html&gt;
	&lt;head&gt;
		&lt;meta charset=&quot;UTF-8&quot; /&gt;
...
<li>事件的回调需要配置在<strong>methods</strong>对象中，最终会在vm上；</li>
<li>methods中配置的函数，<strong>不要用箭头函数</strong>！否则this就不是vm了；</li>
<li>methods中配置的函数，都是被Vue所管理的函数，<strong>this的指向是vm 或 组件实例对象</strong>；</li>
...
<p><strong>注</strong>：Vue未提供别名的按键，可以使用按键原始的key值去绑定，但注意要转为kebab-case（短横线命名）。</p>
<p><strong>系统修饰键（用法特殊）</strong>：ctrl、alt、shift、meta</p>
<ul>
<li>配合keyup使用：按下修饰键的同时，再按下其他键，随后释放其他键，事件才被触发。</li>
<li>配合keydown使用：正常触发事件。</li>
</ul>
<p><strong>注</strong>：也可以使用keyCode去指定具体的按键（不推荐）</p>
<p><strong>注</strong>：Vue.config.keyCodes.自定义键名 = 键码，可以去定制按键别名</p>
<pre><code class="language-html">&lt;!DOCTYPE html&gt;
&lt;html&gt;
	&lt;head&gt;
		&lt;meta charset=&quot;UTF-8&quot; /&gt;
...
<ul>
<li>初次读取时会执行一次。</li>
<li>当依赖的数据发生改变时会被再次调用。
<strong>优势</strong>：与methods实现相比，内部有缓存机制（复用），效率更高，调试方便。
<strong>注</strong>：</li>
...
<li>监视的属性<strong>必须存在</strong>，<strong>才</strong>能进行监视！！</li>
<li>监视的两种写法：</li>
<li>
<ul>
<li>1.<strong>new Vue时传入watch配置</strong></li>
<li>2.<strong>通过vm.$watch监视</strong></li>
</ul>
</li>
</ul>
<pre><code class="language-html">&lt;!DOCTYPE html&gt;
...
<li>配置deep:true可以监测对象内部值改变（多层）。
<strong>注</strong>：
(1).Vue自身可以监测对象内部值的改变，但Vue提供的watch默认不可以！
(2).使用watch时根据数据的具体结构，决定是否采用深度监视。</li>
</ul>
<pre><code class="language-html">&lt;!DOCTYPE html&gt;
&lt;html&gt;
	&lt;head&gt;
...
<h2>绑定样式</h2>
<h3>1.class样式</h3>
<p>写法:class=&quot;xxx&quot; xxx可以是字符串、对象、数组。
<strong>字符串写法</strong>适用于：类名不确定，要动态获取。
<strong>对象写法</strong>适用于：要绑定多个样式，个数不确定，名字也不确定。
<strong>数组写法</strong>适用于：要绑定多个样式，个数确定，名字也确定，但不确定用不用。</p>
<h3>2.style样式</h3>
<p>:style=&quot;{fontSize: xxx}&quot;	：其中xxx是动态值。
:style=&quot;[a,b]&quot;	：其中a、b是样式对象。</p>
<pre><code class="language-html">&lt;!DOCTYPE html&gt;
&lt;html&gt;
	&lt;head&gt;
		&lt;meta charset=&quot;UTF-8&quot; /&gt;
...
<p><strong>适用于</strong>：切换频率较低的场景。
<strong>特点</strong>：不展示的DOM元素直接被<strong>移除</strong>。
<strong>注意</strong>：v-if可以和v-else-if、v-else一起使用，但要求<strong>结构不能被“打断”</strong>。</p>
<h3>2.v-show</h3>
<p><strong>写法</strong>：v-show=&quot;表达式&quot;
<strong>适用于</strong>：切<strong>换频率较高</strong>的场景。
<strong>特点</strong>：不展示的DOM元素<strong>未被移除</strong>，仅仅是使用样式<strong>隐藏</strong>掉
<strong>备注</strong>：使用v-if的时，元素<strong>可能无法获取到</strong>(如别移除掉的时候)，而使用v-show一定可以获取到。</p>
<pre><code class="language-html">&lt;!DOCTYPE html&gt;
&lt;html&gt;
	&lt;head&gt;
		&lt;meta charset=&quot;UTF-8&quot; /&gt;
...
<ul>
<li>用于展示列表数据</li>
<li><strong>语法</strong>：v-for=&quot;(item, index) in xxx&quot; :key=&quot;yyy&quot;</li>
<li><strong>可遍历</strong>：<strong>数组、对象</strong>、字符串（用的很少）、指定次数（用的很少）</li>
</ul>
<pre><code class="language-html">&lt;!DOCTYPE html&gt;
&lt;html&gt;
	&lt;head&gt;
...
<h3>2.key的原理</h3>
<p><strong>面试题</strong>：react、vue中的key有什么作用？（key的内部原理）
<strong>虚拟DOM中key的作用</strong>：
key是虚拟DOM对象的标识，当数据发生变化时，Vue会根据【新数据】生成【新的虚拟DOM】, 随后Vue进行【新虚拟DOM】与【旧虚拟DOM】的差异比较，比较规则如下。</p>
...
<h3>3.列表过滤</h3>
<blockquote>
<p>此案例中有两种方法，<strong>计算属性</strong>（computed）和<strong>监视</strong>（watch）</p>
</blockquote>
<pre><code class="language-html">&lt;!DOCTYPE html&gt;
&lt;html lang=&quot;en&quot;&gt;
&lt;head&gt;
...
<p>(2).重新解析模板，进而更新页面。</p>
<h3>4.在Vue修改数组中的某个元素一定要用如下方法：</h3>
<p>(1).使用这些API:<strong>push()、pop()、shift()、unshift()、splice()、sort()、reverse()</strong></p>
<p>(2).Vue.set() 或 vm.$set()</p>
<p><strong>特别注意</strong>：Vue.set() 和 vm.$set() 不能给vm 或 vm的根数据对象 添加属性！！！</p>
<pre><code class="language-html">&lt;!DOCTYPE html&gt;
&lt;html&gt;
	&lt;head&gt;
		&lt;meta charset=&quot;UTF-8&quot; /&gt;
...
<h2>收集表单数据</h2>
<ul>
<li>1.&lt; input type=&quot;text&quot;/&gt;，则v-model收集的是value值，用户输入的就是value值。</li>
<li>2.&lt; input type=&quot;radio&quot;/&gt;，则v-model收集的是value值，且要给标签配置value值。</li>
<li>3.&lt; input type=&quot;checkbox&quot;/&gt;
<ul>
<li>没有配置input的value属性，那么收集的就是checked（勾选 or 未勾选，是布尔值）</li>
<li>配置input的value属性:
1)v-model的初始值是非数组，那么收集的就是checked（勾选 or 未勾选，是布尔值）
(2)v-model的初始值是数组，那么收集的的就是value组成的数组</li>
</ul>
</li>
</ul>
<p><strong>备注</strong>：v-model的三个修饰符：
lazy：失去焦点再收集数据
number：输入字符串转为有效的数字
trim：输入首尾空格过滤</p>
<pre><code class="language-html">&lt;!DOCTYPE html&gt;
...
<h2>指令</h2>
<blockquote>
<p>v-bind	: 单向绑定解析表达式, 可简写为 :xxx
v-model	: 双向数据绑定
v-for  	: 遍历数组/对象/字符串
v-on   	: 绑定事件监听, 可简写为@
v-if 	 	: 条件渲染（动态控制节点是否存存在）
v-else 	: 条件渲染（动态控制节点是否存存在）
v-show 	: 条件渲染 (动态控制节点是否展示)</p>
</blockquote>
<h3>1.v-text</h3>
<blockquote>
<p><strong>作用</strong>：向其所在的节点中渲染文本内容。
与插值语法的<strong>区别</strong>：v-text会<strong>替换掉节点中的内容</strong>，{{xx}}则不会。</p>
</blockquote>
<pre><code class="language-html">&lt;!DOCTYPE html&gt;
&lt;html&gt;
	&lt;head&gt;
...
<h3>2.v-html</h3>
<blockquote>
<p><strong>作用</strong>：向指定节点中渲染包含html结构的内容。
与插值语法的<strong>区别</strong>：
(1).v-html会替换掉节点中所有的内容，{{xx}}则不会。
(2).v-html可以识别html结构。</p>
</blockquote>
<p><strong>注</strong>：v-html有安全性问题！！！！</p>
<ul>
...
<h3>4.v-once</h3>
<blockquote>
<p>v-once所在节点在初次动态渲染后，就视为静态内容了。
以后数据的改变不会引起v-once所在结构的更新，可以用于优化性能。</p>
</blockquote>
<pre><code class="language-html">&lt;!DOCTYPE html&gt;
&lt;html&gt;
	&lt;head&gt;
...
<blockquote>
<p>跳过其所在节点的编译过程。
可利用它跳过：没有使用指令语法、没有使用插值语法的节点，会加快编译。</p>
<p>不解析。</p>
</blockquote>
<pre><code class="language-html">&lt;!DOCTYPE html&gt;
&lt;html&gt;
...
<p><strong>配置对象中常用的3个回调</strong>：</p>
<p>(1).bind：指令与元素成功绑定时调用。
...
<li><strong>beforeDestroy</strong>: 清除定时器、解绑自定义事件、取消订阅消息等【收尾工作】。</li>
</ul>
